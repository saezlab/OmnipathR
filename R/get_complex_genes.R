...
#' @param total_match [default=FALSE] logical indicating if the user wants to get all 
#' the complexes where any of the input genes participate (FALSE) or
  ...
  if (total_match){
    complexes_geneset <- 
      complexes[which(unlist(lapply(strsplit(complexes$components_genesymbols,'-'), 
                                    function(x){
                          sum(x %in% query_genes) == length(query_genes)}))),]  
  } else {
  complexes_geneset <- 
    complexes[which(unlist(lapply(strsplit(complexes$components_genesymbols,'-'), 
