...
import_TFregulons_Interactions = function (from_cache_file=NULL,
  ...
  if (select_organism %in% c(9606, 10116, 10090)){
    if (select_organism == 9606){
      url <- 'http://omnipathdb.org/interactions?datasets=tfregulons&fields=sources,tfregulons_level&genesymbols=1'    
    } else {
      if (select_organism == 10116){
        url <- 'http://omnipathdb.org/interactions?datasets=tfregulons&fields=sources,tfregulons_level&genesymbols=1&organisms=10116'
      }
      if (select_organism == 10090){
        url <- 'http://omnipathdb.org/interactions?datasets=tfregulons&fields=sources,tfregulons_level&genesymbols=1&organisms=10090'
      }
    }     
  } else {
    stop("The selected organism is not correct")
...
#' \link{import_KinaseExtra_Interactions}, \link{import_LigrecExtra_Interactions},
#' \link{import_miRNAtarget_Interactions}, \link{import_TFregulons_Interactions}}
...
import_Omnipath_complexes = function (from_cache_file=NULL,
  ...
  # we remove references mentioned multiple times:
  filteredcomplexes$references = unlist(lapply(strsplit(filteredcomplexes$references,split = ";"),
    function(x)paste(unique(x),collapse=";")))
  filteredcomplexes$nsources =	unlist(lapply(strsplit(filteredcomplexes$sources,split = ";"),length))
  filteredcomplexes$nrefs =	unlist(lapply(strsplit(filteredcomplexes$references,split = ";"),length))
...
import_Omnipath_annotations = function (from_cache_file=NULL,
  ...
  if(is.null(from_cache_file)){
    annotations = read.csv(url_annotations, sep = '\t', header = TRUE,
                           stringsAsFactors = F)
...
.get_annotation_databases = function(){
  ...
  annotations_db <- c("Adhesome","SignaLink3","TopDB","HGNC","Ramilowski_location", 
                      "Vesiclepedia","HPMR","Kirouac2010","Surfaceome","Signor","Locate","CellPhoneDB",
                      "Membranome","MatrixDB", "Matrisome","GO_Intercell","Ramilowski2015","Integrins",
                      "CSPA","Guide2Pharma","Zhong2015","HPA","NetPath","ComPPI","KEGG","CPAD","Exocarta",
...
########## SOURCE FILTERING      ##########   
########## ########## ########## ##########
# filtering the PTMS, interactions and complexes which are reported in the 
# import functions
.filter_sources = function(interactions, databases){
# takes interactions, PTMS or complexes and removes interactions which are
...
# filtering the annotations which are reported in the import functions
.filter_sources_annotations = function(annotations, databases){
  # takes annotations and removes interactions which are
  # not reported by the asked databases.
...
# filtering the intercell records according to the categories selected
.filter_categories_intercell = function(intercell, categories){
  ...
  subsetIntercell <- dplyr::filter(intercell, category %in% categories)
